// Vendored from open-design's apps/daemon/src/design-systems/showcase.ts at open-design-v0.22.2
// (commit 73953213a6fe). Apache-2.0 — see ./SOURCE.md.
// LOCAL DEVIATION (the only one): the optional `resolvedTokens` argument
// and the `resolvedTokens?.x ??` prefixes on the palette/font picks below.
...
/**
 * LOCAL DEVIATION: values resolved from the design system's tokens.css (see
 * core's resolveDesignSystemTokens). Each one given replaces the matching
 * heuristic pick from DESIGN.md prose; omitted ones keep upstream behavior.
 * Values are inserted into a <style> block as-is, so callers must pass
 * sanitized CSS values.
 */
export interface ShowcaseResolvedTokens {
  bg?: string;
  fg?: string;
  accent?: string;
  accentFg?: string;
...
export function renderDesignSystemShowcase(id: string, raw: string, resolvedTokens?: ShowcaseResolvedTokens): string {
  ...
  const display = resolvedTokens?.display ?? fonts.display ?? fonts.heading ?? "system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif";
  const body = resolvedTokens?.body ?? fonts.body ?? display;
  const mono = resolvedTokens?.mono ?? fonts.mono ?? "ui-monospace, 'JetBrains Mono', monospace";

  const accentFg = resolvedTokens?.accentFg ?? pickReadableForeground(accent);
