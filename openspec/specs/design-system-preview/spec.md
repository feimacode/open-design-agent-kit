...
## Purpose
Let a user see any design system (built-in or custom) before committing to it: a Design Systems tree view and a read-only, two-tab preview panel (Visualize, Showcase, plus a DESIGN.md | tokens.css source panel) rendered from one resolved token set per design system, with `tokens.css` as the canonical token source. Covers token resolution, the vendoring of upstream `tokens.css` with additive local overrides, and generating a `tokens.css` for custom systems that lack one.
## Requirements
