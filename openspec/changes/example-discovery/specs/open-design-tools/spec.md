...
`list_open_design_skills` SHALL additionally support an exact `source` filter (matching `skill`, `design-template`, or `example`) and a `remixableOnly` boolean filter (matching only entries with a non-empty `exampleArtifactPath`), both composable with the existing `query` and `mode` filters, so a caller can cheaply narrow the catalog to only its remixable examples without scanning the full merged catalog.
...
#### Scenario: Filtering by exact source
- **WHEN** `list_open_design_skills` is invoked with `source: "example"`
- **THEN** only entries whose `source` is `example` SHALL be returned

#### Scenario: Filtering to remixable entries only
- **WHEN** `list_open_design_skills` is invoked with `remixableOnly: true`
- **THEN** only entries with a non-empty `exampleArtifactPath` SHALL be returned, regardless of their `source` value
