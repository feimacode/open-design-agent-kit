...
- **Social posts, exported.** `export_open_design_artifact` renders a registered artifact in a headless browser (an already-installed Chrome, Edge, or Chromium, found automatically or set with `OPEN_DESIGN_BROWSER_PATH`; nothing is downloaded) and writes upload-ready PNG/JPEG under the artifact's `exports/` folder. It is sized from the source skill (e.g. `card-twitter` → 1600×900), writes one numbered image per `[data-od-card]` card for carousels, and re-encodes to JPEG when a file is over a byte budget (`maxBytes`). The `open-design-social-post` prompt/skill wraps the whole flow: it asks for the platform (X, Instagram, LinkedIn, Xiaohongshu, Stories/Reels, YouTube thumbnail or video), picks the size and skill, generates, and exports. YouTube videos use the HyperFrames template, whose brief replaces upstream's daemon render with a direct `npx hyperframes render` to MP4 (needs FFmpeg). For scripts and CI, the CLI has `export` and `render-video` commands.
- **Decks to PowerPoint and PDF.** The same `export_open_design_artifact` turns a deck into a `.pptx` (one full-bleed slide image per slide, pixel-perfect but not editable text) or a `.pdf` (one page per slide), and an ordinary page into a vector PDF with selectable text via the browser's print engine. Slide capture is ported from upstream Open Design's desktop exporter: the same slide selectors, presenter chrome hidden, animations frozen, the deck's own slide size measured (4:3 and square decks stay that shape), one slide shown at a time, and off-screen carousel slides restacked. `<deck-stage>` decks that ship without their runtime get upstream's fallback injected into the served copy. Decks are detected from the registered kind or an `od:deck:*` source skill; `deck: true` forces it, and `slides: [1, 3]` exports just those slides. Tested against all 54 vendored deck examples.
